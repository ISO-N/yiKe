...
/**
 * 导出模式显式建模后，页面与网页后台就能共用同一套“完整/增量”的文件命名与执行语义。
 */
...
interface BackupOperations {
     ...
     */
    fun createSuggestedFileName(mode: BackupExportMode = BackupExportMode.FULL): String

    /**
     * 导出到 Uri 是设置页真正触发的高风险动作，因此保留显式接口。
     */
    suspend fun exportToUri(uri: Uri, mode: BackupExportMode = BackupExportMode.FULL)
