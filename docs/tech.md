...
### 6.4 录入页复习间隔配置（ReviewPreviewPanel）

为避免“预览面板交互触发 Provider 写入 → 页面级重建 → 输入内容丢失”的问题，录入页复习配置采用“本地草稿 + 显式保存”模式：

1. **面板内交互仅更新草稿**：Slider/Switch/增减轮次/恢复默认等操作只修改本地 `_draft`，不触发 `ReviewIntervalsNotifier.save()`。
2. **显式保存**：用户点击“确认保存”时，才调用 `ReviewIntervalsNotifier.save(_draft)` 持久化到设置表。
3. **离开拦截**：`InputPage` 使用 `PopScope` 在存在未保存更改时拦截返回，并弹出确认弹窗，防止误丢弃草稿。
4. **配置合法性校验（双保险）**：
   - UI 层会在面板内即时提示并禁用“确认保存”
   - Provider 层在 `save()` 前再次校验，阻止非法配置落库（避免出现“第 1 轮 +100 天、第 2 轮 +1 天”这类后轮早于前轮的情况）

校验规则要点：

- 至少保留 1 轮启用
- 启用轮次的 `intervalDays` 必须严格递增（保证后一次复习一定在前一次之后）
