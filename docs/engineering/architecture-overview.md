...
目前代码已经把首页概览、问题搜索、复习评分、问题编辑正式保存等主链路逐步迁入 `domain/usecase/`，
其余页面仍允许沿用 Repository 直连方式，但新增或重构业务动作时应优先落在 UseCase 中继续收口。
...
- `shell`：顶部玻璃导航、上下文栏、全局状态芯片、移动端折叠菜单与跨工作区返回路径
