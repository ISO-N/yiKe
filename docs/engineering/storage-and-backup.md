...
当前工程同时允许用户在“网页后台”中导出同一份 JSON 备份，
这样同一局域网或热点下的电脑浏览器也能直接下载本机数据快照。
...
当前工程同时允许用户在“网页后台”页上传 JSON 备份并执行同一条恢复链路，
浏览器上传不会改变恢复语义，仍然是“校验后全量覆盖本机数据”。
...
- 网页后台中的备份导出与恢复复用同一 `BackupService`，浏览器上传 JSON 后仍执行相同的校验、事务与提醒重建
